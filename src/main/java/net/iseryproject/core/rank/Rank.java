package net.iseryproject.core.rank;
...
public class Rank {
	...
	public void save() {
		...
		data.put("uuid", String.valueOf(uuid));
		data.put("name", displayName);
		Core.get().getPidgin().sendMessage(new Message("RANK_REFRESH", data));
	}
	...
	public void delete() {
		...
		Map<String, Object> data = new HashMap<>();
		data.put("uuid", String.valueOf(uuid));
		Core.get().getPidgin().sendMessage(new Message("RANK_DELETE", data));
	}
